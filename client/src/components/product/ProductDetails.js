...
const ProductDetails = () => {
  ...
  const { isLoading: isProductLoading, data: productDetails } =
    useGetProductDetails(productId);
  const {
    mutate: postReviewMutate,
  ...
  const handleAddToCart = () => {
    ...
    dispatch(
      addToCart({ product: productDetails?.data.product, userId, quantity })
    );
  };
      ...
      {isProductLoading ? (
        ...
        <>
          <div className="product-details mx-6 my-4">
            <div className="mx-24 flex flex-col lg:flex-row xl:flex-row justify-center items-center md:justify-start md:items-start lg:gap-5 xl:gap-5 xl:justify-between">
              
              <div className="mb-56 w-[500px] bg-red-100 ">
                <Slider {...settings}>
                  {productDetails.data.product.images.map((image, index) => (
              ...
              <div className="py-5 space-y-3 lg:w-2/5 xl:w-[500px]">
                ...
                <hr />
                {productDetails.data.product.reviews.length > 0 ? (
                  <div className="flex gap-3 items-center">
                    <div className="rating-outer">
                      <div
                        ...
                        style={{
                          width: `${
                            (productDetails.data.product.averageRating / 5) *
                            100
                          }%`,
                        }}
                      ></div>
                    ...
                    <span id="no_of_reviews">
                      ({productDetails.data.product.reviews.length}{" "}
                      {productDetails.data.product.reviews.length === 1
                        ? "Review"
                        : "Reviews"}
                      )
                    </span>
